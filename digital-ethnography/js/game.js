...
function setup() {
  ...
  p1 = new Planet1(100,int(windowHeight-300), c1= color(176, 119, 250), c2= color(143, 15, 118));
  p2 = new Planet2(windowWidth/2-100, (windowHeight-300), c1=color(32, 154, 229), c2=color(97, 255, 235));
  alien = new Alien(windowWidth/4, int(windowHeight-300), 50);

  bg_col = color(10, 9, 41);
}

function draw() {
  canvas.style = "margin-top: 2975px;";
...
class Planet1{
  ...
  shoot_laser(x, y){
    ...
    if(frameCount % 10 == 0 && playerDist < this.d+50){
  ...
  spin_planet() {
      ...
      for(let x = this.x-r; x < this.x+r; x+=step){
        for(let y = this.y-r; y < this.y+r; y+=step){
          ...
          if(length < r)
          {
            n = noise(x * scale + frameCount * 0.005, y * scale * 5 + frameCount *0.005);
            n = int(n*10)/10;
